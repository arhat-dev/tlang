...
var execTests = []execTest{
	...
	{"dot bool", "`<`\n.\n`>`", "<true>", true, true},
	{"dot complex", "`<`\n . \n`>`", "<(16.2-17i)>", 16.2 - 17i, true},
	{"dot string", "'<' . '>'", "<hello>", "hello", true},
	{"dot slice", "`<`\n . \n`>`", "<[-1 -2 -3]>", []int{-1, -2, -3}, true},
	{"dot map", "`<`\n. \n`>`", "<map[two:22]>", map[string]int{"two": 22}, true},
	{"dot struct", "`<`\n .\n`>`", "<{7 seven}>", struct {
		a int
		b string
	}{7, "seven"}, true},
...
func TestEvalFieldErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			".MissingField",
			(*T)(nil),
			"can't evaluate field MissingField in type *tlang.T",
		},
		{
		...
		},
		{
			...
			".X",
			(*T)(nil),
			"nil pointer evaluating *tlang.T.X",
		},
		{
			"MissingKeyOnNilMap",
			".MissingKey",
